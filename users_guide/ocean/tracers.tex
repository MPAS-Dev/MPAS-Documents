...
Both active and passive tracers in MPAS-Ocean are governed by equation \ref{ocean:tracer}.  In the current version of the code, there are 5 specific groups of tracers:
\vskip 0.2cm

\noindent{{\bf{activeTracers}}. Potential temperature and salinity.} 
\vskip 0.2cm

\noindent{{\bf{debugTracers}}. A passive tracer used primarily for testing conservation and monotonicity of tracer advection.}
\vskip 0.2cm

\noindent{{\bf{ecosysTracers}}. Ocean biogeochemistry and ecosystem dynamics based on the Biogeochemical Elemental Cycling (BEC) model of Moore.}
...
\noindent{{\bf{DMSTracers}}. Extension of BEC for prognostic sulfur cycling focused on calculating the flux of dimethyl sulfide (DMS) from the ocean to the atmosphere.}
\vskip 0.2cm

\noindent{{\bf{MacroMoleculesTracers}}. Extension of BEC that computes the evolution of subclasses of dissolved organic carbon (DOC), such as lipids, proteins, and poly saccharides, that can affect air-sea transfer and the formation of cloud condensation nuclei.}
\vskip 0.2cm
...
\noindent{{\bf{surface\_bulk\_forcing}}. Surface flux will be applied.  It can be supplied by reading in a field from a file, calculating in a user-created routine, passed from another model component (for example, by the E3SM flux coupler), or a combination of these.} 
\vskip 0.2cm

\noindent{{\bf{surface\_restoring}}. Surface layer tracer is linearly restored to a climatological field.} 
\vskip 0.2cm

\noindent{{\bf{interior\_restoring}}. Full 3D tracer is linearly restored to a climatological field, except for the surface layer.} 
\vskip 0.2cm

\noindent{{\bf{exponential\_decay}}. Tracer will decay at an exponential rate.  Intended for radioactive tracers and, for example, simple models of oil degradation.} 
\vskip 0.2cm

\noindent{{\bf{ideal-Age\_forcing}}. Tracer is Ideal Age, where the surface value is reset to zero and interior values are incremented by {\it dt} every timestep.} 
\vskip 0.2cm

\noindent{{\bf{ttd\_forcing}}. Tracer surface field is reset to a specified value every timestep.  Intended for simulating Time Transit Distribution functions (TTDs).} 
\vskip 0.2cm
