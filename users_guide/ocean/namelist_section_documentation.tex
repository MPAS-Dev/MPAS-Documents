...
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    ...
    \hline
    Possible Values: & 'uniform\_stretching', 'fixed', 'user\_specified', 'impermeable\_interface' \\
    \hline
    \caption{config\_vert\_coord\_movement: Determines the vertical coordinate movement type. 'uniform\_stretching' distributes SSH perturbations through all vertical levels (z-star vertical coordinate); 'fixed' places them all in the top level (z-level vertical coordinate); 'user\_specified' allows the input file to determine the distribution using the variable vertCoordMovementWeights (weighted z-star vertical coordinate); and 'impermeable\_interface' makes the vertical transport between layers zero (idealized isopycnal).}
\end{longtable}
\end{center}
\subsection[config\_use\_minimum\_thickness]{\hyperref[sec:nm_tab_ALE_vertical_grid]{config\_use\_minimum\_thickness}}
\label{subsec:nm_sec_config_use_minimum_thickness}
\begin{center}
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    \hline
    Type: & logical \\
    ...
    Possible Values: & .true. or .false. \\
    \hline
    \caption{config\_use\_minimum\_thickness: If true, a minimum thickness is enforced to prevent massless layers.}
\end{longtable}
\end{center}
\subsection[config\_minimum\_thickness]{\hyperref[sec:nm_tab_ALE_vertical_grid]{config\_minimum\_thickness}}
\label{subsec:nm_sec_config_minimum_thickness}
\begin{center}
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    \hline
    Type: & real \\
    ...
    \hline
    \caption{config\_minimum\_thickness: Minimum thickness allowed.}
\end{longtable}
\end{center}
\subsection[config\_set\_restingThickness\_to\_z]{\hyperref[sec:nm_tab_ALE_vertical_grid]{config\_set\_restingThickness\_to\_z}}
\label{subsec:nm_sec_config_set_restingThickness_to_z}
\begin{center}
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    \hline
    Type: & logical \\
    ...
    Possible Values: & .true. or .false. \\
    \hline
    \caption{config\_set\_restingThickness\_to\_z: If true, compute restingThickness using refBottomDepth.  This can be used for z-type grids.  If false, restingThickness must be in init or restart file.  For sigma grids, this must be false.}
\end{longtable}
\end{center}
...
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    ...
    Possible Values: & .true. or .false. \\
    \hline
    \caption{config\_use\_freq\_filtered\_thickness:  If true,  $h^{hf}$  is included in the desired ALE thickness in \ref{ocean:desired thickness}, and the prognostic equations \ref{Dlf} and \ref{hhf} are integrated in the code.}
\end{longtable}
\end{center}
...
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    ...
    \hline
    \caption{config\_thickness\_filter\_timescale:  Filter timescale for the low-frequency baroclinic divergence,  $\tau_{Dlf}$ .}
\end{longtable}
\end{center}
...
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    ...
    Possible Values: & .true. or .false. \\
    \hline
    \caption{config\_use\_highFreqThick\_restore:  If true, the high frequency thickness equation (\ref{ocean:hhf}) includes term 2 on the RHS, the restoring term.  The high frequency thickness is restored to zero with timescale  $\tau_{hhf}$ .}
\end{longtable}
\end{center}
...
\begin{longtable}{| p{2.0in} | p{4.0in} |}
    ...
    \hline
    \caption{config\_highFreqThick\_restore\_time:  Restoring timescale for thickness,  $\tau_{hhf}$ .}
\end{longtable}
\end{center}
